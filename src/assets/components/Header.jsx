        ...
        <div className="shadow-md  w-full fixed top-0 left-0">
            <div className="md:px-10 py-4 px-7 md:flex justify-between items-center bg-black">
                ...
                <ul className={`md:flex mx-auto md:items-center md:pb-0 pb-12 absolute md:static md:z-auto z-[-1]
                 left-0 w-full md:w-auto md:pl-0 pl-9 transition-all bg-black duration-500 ease-in  ${isOpen ? 
                 'top-12' : 'top-[-499px]'}`}>
                    {Links.map((link, index) => (
                        <li
                            key={index}
                            className={`font-normal my-7 md:my-0 md:ml-8 ${activeLink === index ? 'text-yellow-500' : 'text-white'}`}
                        >
